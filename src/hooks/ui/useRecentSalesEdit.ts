...
export const useRecentSalesEdit = (
  ...
  const [editUsdPaymentRef, setEditUsdPaymentRef] = useState<number | null>(
    null,
  );
  ...
  const startEdit = (sale: Sale) => {
    ...
    const usdPayment = payments.find(
      (p) =>
        p.currency === "USD" || p.method_id === "usd" || p.methodId === "usd",
    );
    setEditUsdPaymentRef(
      usdPayment
        ? (usdPayment.amount_ref ?? usdPayment.amountRef ?? null)
        : null,
    );
  };
