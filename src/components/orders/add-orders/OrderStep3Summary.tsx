...
import { CartItem, Payment } from "@/shared/types";
import {
  CheckCircle2,
  Package,
  User,
...
export function OrderStep3Summary({
  ...
  const totalAdvancesUsd = payments.reduce(
    (acc, p) => acc + (p.amountRef || 0) + (p.amountBs / tasa || 0),
    0,
  );
  const remainingUsd = Math.max(0, subtotalUsd - totalAdvancesUsd);

  const paymentPercentage =
    subtotalUsd > 0 ? (totalAdvancesUsd / subtotalUsd) * 100 : 0;
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-zinc-100 rounded-3xl p-4 shadow-sm relative overflow-hidden group">
          ...
          <h4 className="font-bold text-zinc-800 text-[11px] mb-2">
            Abonado del Total
          </h4>
      ...
      <div className="bg-linear-to-br from-primary-500 to-primary-700 rounded-3xl p-6 text-white shadow-xl shadow-primary-500/20 relative overflow-hidden active:scale-[0.98] transition-transform cursor-default">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary-100">
              Valor del Pedido
            </span>
          ...
          <div className="text-right bg-white/10 backdrop-blur-md px-3 py-2 rounded-2xl border border-white/20">
            <p className="text-[9px] font-bold uppercase tracking-widest text-primary-100 mb-0.5">
              Bolívares
            </p>
            <p className="text-lg font-black leading-none">
              Bs{" "}
              {(subtotalUsd * tasa).toLocaleString("es-ES", {
        ...
        <div className="pt-4 border-t border-white/10 flex justify-between items-center">
          <div className="flex gap-4">
            <div className="flex flex-col">
              <span className="text-[9px] font-bold text-primary-200 uppercase tracking-tighter">
                Abonado
              </span>
              <span className="text-sm font-black text-white">
                ${totalAdvancesUsd.toFixed(2)}
              </span>
            </div>
            <div className="flex flex-col">
              <span className="text-[9px] font-bold text-primary-200 uppercase tracking-tighter">
              ...
              <span className="text-sm font-black text-white/90 underline decoration-red-400 decoration-2 underline-offset-2">
                ${remainingUsd.toFixed(2)}
              </span>
            </div>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-[9px] font-bold text-primary-200 uppercase tracking-tighter text-right">
              Tasa BCV del día
            </span>
      ...
      <div className="flex flex-col items-center text-center gap-2 py-2">
        ...
        <p className="text-[10px] text-zinc-500 max-w-[220px] font-medium leading-relaxed">
          Los anticipos se registrarán automáticamente en la caja del turno
          actual.
        </p>
      </div>
    </motion.div>
